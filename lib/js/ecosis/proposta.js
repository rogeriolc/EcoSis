...
function setNrProtocolo(el) {
    let valor = $(el).val();

    proposta.nr_protocolo = valor;

    window.localStorage.setItem("proposta", JSON.stringify(proposta));
}

function setNrAlteracao(el) {
    let valor = $(el).val();

    proposta.nr_alteracao = valor;
...
function renderPropostaData() {
    ...
    $("#formNewProposta input[name=nrProtocolo]").val(proposta.nr_protocolo);
    $("#formNewProposta input[name=nrAlteracao]").val(proposta.nr_alteracao);
