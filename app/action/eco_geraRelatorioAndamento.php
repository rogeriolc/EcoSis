...
$cdCliente = isset($_GET['cdCliente']) ? base64_decode($_GET['cdCliente']) : null;
$cdServico = isset($_GET['cdServico']) ? base64_decode($_GET['cdServico']) : null;
$tpStatus  = isset($_GET['tpStatus']) ? base64_decode($_GET['tpStatus']) : null;
...
if (is_null($cdCliente) || is_null($cdServico)) {
	echo 'Parâmentros incorretos!';
	exit();
}
...
";
$sql .= (!is_null($tpStatus)) ? " AND it.tp_status IN ($tpStatus)" : null;
$sql .= " ORDER BY it.cd_it_atividade DESC";
$stmt = $mysql->prepare($sql);
$stmt->bindParam(":cdCliente", $cdCliente);
$stmt->bindParam(":cdServico", $cdServico);
// (!is_null($tpStatus)) ? $stmt->bindValue(":tpStatus", $tpStatus) : null;
...
foreach ($dataHead as $key => $value) {
	...
	foreach ($atividades as $akey => $avalue) {
		...
		foreach ($andamentos as $adkey => $advalue) {
			...
			$html .= '</tr>';
			$html .= '<tr>';
			$html .= '<td align="center" colspan="2" style="background-color: #E9E9E9">'.$advalue['tp_status'].'</td>';
			$html .= '</tr>';
			$html .= '<tr>';
